...
class Order:
    ...
    def __init__(self, user_name:str, city:str, postoffice:int,\
         items:list, vehicle:None=None, orderId:int=None, location:Location=None):
...
class LogisticSystem:
    ...
    def placeOrder(self, order:Order):
        ...
        if len([vehicle for vehicle in self.vehicles if vehicle.isAvailable is True]) == 0: # no vehicles available
    ...
    def trackOrder(self, orderId:int) -> str:
        ...
        if orderId not in self.id_dict:
            print("No such order.")
            return
        obj = ctypes.cast(orderId, ctypes.py_object).value   # access the Order object by id
        print(f"Your order #{orderId} is sent to {obj.city}. Total price: {obj.calculate_Amount()} UAH.")
...
def additional_test():
    ...
    extra_order = Order(user_name = 'Marta', city = 'Ternopli', postoffice = 7, items = other_items)
